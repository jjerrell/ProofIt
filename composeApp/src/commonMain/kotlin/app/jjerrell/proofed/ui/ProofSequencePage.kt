...
fun ProofSequencePage(
    ...
    val isLoadingOrError =
        viewModel.state is ProofSequencePageViewModel.State.Loading ||
            viewModel.state is ProofSequencePageViewModel.State.Error
    if (viewModel.state == null) {
        LaunchedEffect(Unit) { viewModel.getSequence(sequenceId) }
    } else {
        LazyColumn(
            modifier = modifier,
            verticalArrangement =
            ...
            when (val capturedState = viewModel.state) {
                ...
                is ProofSequencePageViewModel.State.Success ->
                    items(items = capturedState.sequence.steps, key = { it.id }) { stepItem ->
                        ProofStepMenuItem(
                            sequence = capturedState.sequence,
                ...
                is ProofSequencePageViewModel.State.Error -> {
                    item {
                        ...
                        TextButton(onClick = { viewModel.getSequence(sequenceId) }) {
                            Text(text = "Try Again")
                        }
                    }
                }
